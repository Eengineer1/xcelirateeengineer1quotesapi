...
class ShoutquoteController extends AbstractController
{
    ...
    public function shout(MessageBusInterface $bus,Request $request,string $author): Response
    {
        ...
        if($limit>10 or $limit<=0){throw new BadRequestHttpException('Prohibited quote limit. You need to specify a limit lower than or equal to (<=) 10 and greater than 0.',null,400);}
        if(in_array($author,['',' ','undefined',null])){throw new BadRequestHttpException('You need to specify a valid author, e.g. Steve Jobs as steve_jobs. You can also call the /shout/list endpoint for a list of valid authors.',null,400);}
        $rediskey="$author?limit=$limit";
        $item=$this->redis->get($rediskey);
        if($item){return new Response($item,Response::HTTP_OK);}
        ...
        if(!$quotes){
            ...
            $theyshouted=[];
            foreach($quotes as $q){$theyshouted[]=$this->generatedshoutedquote($q->getQuote());}
        }
    ...
    public function list(): Response
    {
        return $this->json([
            ...
            'path' => 'src/Controller/ShoutquoteController.php',
        ]);
    }
    /**
     * @param string $quote
